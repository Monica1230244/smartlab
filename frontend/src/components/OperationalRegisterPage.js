      ...
      <div className="reclamLayout">
        <main className="reclamMain">
          <section className="dashPanel reclamTablePanel">
            ...
            {viewMode === 'table' ? (
              <div className="tableScroll"><table><thead><tr>{config.columns.map((column) => <th key={column.name || column.label}>{column.label}</th>)}<th>Actions</th></tr></thead><tbody>{filtered.map((record) => <tr key={record.id} className={selected?.id === record.id ? 'selectedRow' : ''} onClick={() => setSelectedId(record.id)}>{config.columns.map((column) => <td key={column.name || column.label}>{column.badge ? <span className={`statusBadge ${statusTone(valueOf(record, column))}`}>{valueOf(record, column)}</span> : valueOf(record, column)}</td>)}<td><div className="rowActions"><button type="button" className="ghostButton iconOnlyButton" onClick={(event) => { event.stopPropagation(); setSelectedId(record.id); }}>o</button>{!config.readOnly && <><button type="button" className="ghostButton iconOnlyButton" onClick={(event) => { event.stopPropagation(); openEdit(record); }}>M</button><button type="button" className="dangerButton" onClick={(event) => { event.stopPropagation(); remove(record); }}>Suppr.</button></>}</div></td></tr>)}{filtered.length === 0 && <tr><td colSpan={config.columns.length + 1} className="emptyCell">Aucune donnee Supabase pour ce module. Ajoutez un enregistrement pour alimenter l'interface.</td></tr>}</tbody></table></div>
            ) : (
