...
function config({ resource, prefix, title, subtitle, workflow, evidence, fields, columns = fields, detailFields = fields }) {
  return {
    ...
    summary: (records) => [
      ...
      { label: 'Actifs', value: records.filter((item) => ['actif', 'en_cours', 'ouverte', 'planifie'].includes(item.statut)).length, tone: 'amber', icon: 'EC' },
      { label: 'Validés', value: records.filter((item) => ['valide', 'conforme', 'termine', 'cloturee', 'atteint'].includes(item.statut)).length, tone: 'green', icon: 'OK' },
      { label: 'Alertes', value: records.filter((item) => ['retard', 'bloque', 'critique', 'non_conforme'].includes(item.statut)).length, tone: 'red', icon: '!' }
    ],
...
export function DemandesPrestations() {
  const fields = [
    ...
    { name: 'responsable', label: 'Responsable' },
    { name: 'statut', label: 'Statut', defaultValue: 'ouverte' }
  ];
...
export function MissionsTerrain() {
  const fields = [
    ...
    { name: 'statut', label: 'Statut', defaultValue: 'planifie' }
  ];
...
export function PlanningProjets() {
  const fields = [
    ...
    { name: 'statut', label: 'Statut', defaultValue: 'en_cours' }
  ];
...
export function MetrologieAvancee() {
  const fields = [
    ...
    { name: 'statut', label: 'Statut', defaultValue: 'conforme' }
  ];
  return <OperationalRegisterPage config={config({ resource: 'metrologieAvancee', prefix: 'MET', title: 'Metrologie avancee', subtitle: 'Etalons, certificats, verifications intermediaires, incertitudes et decisions de conformite.', workflow: ['Inventaire', 'Verification', 'Etalonnage', 'Decision', 'Suivi periodique'], evidence: ['Certificat', 'Incertitude', 'Decision', 'Traçabilite SI'], fields })} />;
}
