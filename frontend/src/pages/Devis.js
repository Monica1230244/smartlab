...
function buildValidationUrl(code) { const base = `${window.location.origin}${(import.meta.env.BASE_URL || '/').replace(/\/$/, '')}`.replace(/\/$/, ''); return `${base}/#/devis?validation=${encodeURIComponent(code || '')}`; }
