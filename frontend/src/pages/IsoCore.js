...
function IsoRegister({ config }) {
  ...
  return (
    <div className="reclamMockPage">
      ...
      <div className="reclamLayout"><main className="reclamMain"><section className="dashPanel reclamTablePanel"><div className="quoteTableTop"><strong>Registre</strong><div className="rowActions"><button type="button" className="ghostButton" onClick={() => downloadCsv(`${config.resource}.csv`, filtered)}>Exporter</button><button type="button" className="primaryButton" onClick={openCreate}>+ Nouveau</button></div></div><div className="tableScroll"><table><thead><tr><th>Reference</th><th>Objet / description</th><th>Responsable</th><th>Statut</th><th>Priorite / score</th><th>Echeance</th><th>Actions</th></tr></thead><tbody>{filtered.map((record) => { const isRisk = config.resource === 'risquesOpportunites'; const [riskLabel, riskToneCls] = isRisk ? riskLevel(record) : ['', '']; return <tr key={record.id} className={selected?.id === record.id ? 'selectedRow' : ''} onClick={() => setSelectedId(record.id)}><td><strong>{record.reference}</strong><br /><small>{record.origine || record.type || record.periode || ''}</small></td><td>{record.objet || record.description || record.decisions || '-'}<br /><small>{record.source || record.cause || record.conclusion || ''}</small></td><td>{record.responsable || '-'}</td><td><span className={`statusBadge ${statusTone(record.statut)}`}>{record.statut || '-'}</span></td><td>{isRisk ? <span className={`statusBadge ${riskToneCls}`}>{riskLabel} ({riskScore(record)})</span> : <span className={`statusBadge ${priorityTone(record.priorite)}`}>{record.priorite || '-'}</span>}</td><td>{record.echeance || record.date_prevue || '-'}</td><td><div className="rowActions"><button type="button" className="ghostButton iconOnlyButton" onClick={(event) => { event.stopPropagation(); openEdit(record); }}>M</button><button type="button" className="dangerButton" onClick={(event) => { event.stopPropagation(); remove(record); }}>Suppr.</button></div></td></tr>; })}{filtered.length === 0 && <tr><td colSpan="7" className="emptyCell">Aucune donnee trouvee</td></tr>}</tbody></table></div></section></main><aside className="reclamRightStack"><section className="dashPanel reclamDetailPanel"><div className="dashPanelHeader"><strong>{selected?.reference || 'Detail'}</strong><span className={`statusBadge ${statusTone(selected?.statut)}`}>{selected?.statut || '-'}</span></div><div className="quoteDetailBody"><h4>Informations</h4><dl>{selected ? Object.entries(selected).filter(([key]) => !['id'].includes(key)).slice(0, 12).map(([key, value]) => <React.Fragment key={key}><dt>{key.replaceAll('_', ' ')}</dt><dd>{typeof value === 'object' ? JSON.stringify(value) : String(value || '-')}</dd></React.Fragment>) : <><dt>Selection</dt><dd>Aucune donnee</dd></>}</dl></div></section></aside></div>
